...
pub fn not_fn(value: &lval::LVal, environment:&mut env::Env) ->lval::LVal{
    match value {
        &lval::LVal::List(ref v) => if v.len() == 1 {
            match v[0].clone(){
                lval::LVal::Bool(b) => lval::LVal::Bool(!b),
                _ => lval::LVal::Error("not_fn must be called on a list with a boolean element".to_string()),
            }
        }else{
            lval::LVal::Error("not_fn must be called on a list with a single element".to_string())
        },
        _ => lval::LVal::Error("not_fn must be called on a list".to_string()),
    }
}
